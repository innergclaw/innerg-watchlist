...
test('news failure leaves public charts usable',async()=>{const status={};await loadNews({querySelector:()=>status},async()=>{throw Error('offline')});assert.match(status.textContent,/charts remain open/);});
...
test('private news is not a public source file',()=>{assert.equal(fs.existsSync('data/asset-news.json'),false);});
