...
test('refresh failure retains usable data',async()=>{const h=harness();await h.settle();h.setFailure(true);await vm.runInContext('loadData()',h.context);assert.ok(h.get('#market-state').textContent.includes('Refresh failed'));assert.equal((h.get('#sector-list').innerHTML.match(/class="asset-card"/g)||[]).length,33);});
