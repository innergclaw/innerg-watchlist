...
Deno.serve(async (req: Request) => {
  ...
  if (!Number.isInteger(amount) || amount < MIN || amount > MAX) {
    return Response.json({ error: "Choose a monthly rate between $7 and $14." }, { status: 400, headers: corsHeaders });
  }
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      mode: "subscription",
      line_items: [{ price_data: { currency: "usd", unit_amount: amount, recurring: { interval: "month" }, product_data: { name: "INNERG Founding Membership" } }, quantity: 1 }],
      customer_email: user.email ?? undefined,
      client_reference_id: user.id,
      success_url: returnUrl,
      cancel_url: "https://innergclaw.github.io/innerg-watchlist/#member-access",
      metadata: { membership_type: "innerg_founding", monthly_amount_cents: String(amount), user_id: user.id },
      subscription_data: { metadata: { membership_type: "innerg_founding", user_id: user.id, monthly_amount_cents: String(amount) } },
    });
