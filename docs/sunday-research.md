...
6. Update `moverContext` in `weekly-mover.mjs` for the asset with the highest finite weekly percentage return in the refreshed snapshot. Review recent Reddit discussion and X posts. Open sources, separate community opinion from reported news, and explain possible drivers in plain language without claiming proven causation. Link each platform's dated sources. If X is only readable as an embedded post in reporting, disclose that and link the report too. Use the actual review timestamp. Never invent unavailable platform coverage. The UI shows only one leader, hides context for a different symbol, and expires context after seven days.
